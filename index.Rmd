...
solicitud_ac_wfs <- 
  "request=GetFeature&service=WFS&version=1.0.0&typename=PNE:areas_conservacion&outputFormat=application/json"

# area de conservacion
cr_ac <-
  st_read(paste0(url_base_wfs_sinac, solicitud_ac_wfs)) %>%
...
leaflet(datos) %>%
  ...
  addPolygons(data = redvial_wgs84, color = "#35b541", fill = TRUE, fillColor = "#35b541", stroke = T, weight = 3, opacity = 0.5,
              group = "Red Vial") %>%
  addLayersControl(baseGroups = c("OpenStreetMap", "Stamen Toner Lite", "CartoDB DarkMatter"),
                   overlayGroups = c("Registros","Distritos con registros", "Densidad", "ASP", "Área de Conservación", "Red Vial"),
                   options = layersControlOptions(collapsed = TRUE))  %>%
  addScaleBar() %>%
  hideGroup(c("Distritos con registros", "Densidad", "ASP", "Área de Conservación", "Red Vial")) %>%
   addMiniMap(
    toggleDisplay = TRUE,
    position = "bottomleft",
